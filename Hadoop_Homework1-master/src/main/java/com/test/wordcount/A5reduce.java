...
//public class A2reduce extends Reducer<Text, Text,Text, FloatWritable> {
public class A5reduce extends Reducer<Text, Text,Text, Text> {
    ...
    public void cleanup(Context context)throws IOException,InterruptedException
    {
        ...
        {
            int len = entry1.getKey().length();
            if(len>=4)
            {
                if(entry1.getKey().substring(len-4,len).equals("4941"))
                {
