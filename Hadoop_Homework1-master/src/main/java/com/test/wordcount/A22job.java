...
public class A22job {
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf,"A22");
        job.setJobName("169171-A22job-"+System.currentTimeMillis());
        job.setJarByClass(A22job.class);
//2. 设置job任务
        // 第一步：设置输入类和输入路径和输出
...
//        FileOutputFormat.setOutputPath(job, new Path("/Users/linyouguang/Desktop/output"));
        FileInputFormat.addInputPath(job,new Path("/user/s1155169171/output-A2-1633785005936/part-r-00000"));
        FileOutputFormat.setOutputPath(job, new Path("/user/s1155169171/output-A2-"+System.currentTimeMillis()));
        // 第二步：设置Mapper类和数据类型
        job.setMapperClass(A22map.class);
